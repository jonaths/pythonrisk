...
risk_profile = [0.0,2.0,4.0]

# Los diferentes presupuestos
budgets = [30,60,120,180]
    ...
    for rp in risk_profile:
        ...
        for i in range(0, episodes):
            ...
            for j in range(0, maxsteps):
                ...
                newQShaped = agent.updateQ(shaped_reward, currentstate_index, newstate_index, action_index)

                # Actualiza la tabla de las recompensas reales
                newQReal = agent.updateQ(current_reward, currentstate_index, newstate_index, action_index, 'real')

                print "newQ:", str(newQShaped)
                print "newQRow:", agent.getQ()[currentstate_index]

                # Actualiza la misma posicion para el siguiente nivel de budget
                next_budget_state_index = currentstate_index - xlim * ylim
                agent.setQCell(next_budget_state_index, action_index, newQShaped)
                agent.setQCell(next_budget_state_index, action_index, newQReal, 'real')
            ...
            with open('rewards.csv', 'a') as f:
                f.write(str(b) + ',' + str(rp) + ',' + str(i) + ',' + str(agent.getAccumulatedReward('real')) + ',' + str(j + 1) + ',' + str(newposition) + '\n')
            ...
            if i != 0 and i % 200 == 0:
                ...
                agent.saveHistoryToJson('Output/history-', maxsteps, rp, i)
